...
function App() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      // Check if we got token and user
      if (!data.token || !data.user) {
        alert('Invalid response from server. Missing token or user data.');
  ...
  // Fetch functions
  const fetchTasks = async (token = null) => {
    try {
  ...
  const fetchTags = async (token = null) => {
    try {
      const authToken = token || localStorage.getItem('token');
